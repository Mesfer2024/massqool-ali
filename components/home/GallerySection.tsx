  ...
  if (typeof navigator !== 'undefined' && navigator.share && navigator.canShare) {
    try {
      ...
      if (isDataUrl(src)) {
        ...
        file = new File([blob], 'massqool-piece.jpg', { type: blob.type });
      } else {
        const res = await fetch(src);
        const blob = await res.blob();
        file = new File([blob], 'massqool-piece.jpg', { type: blob.type });
          ...
          {filtered.map((item, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-charcoal/0 group-hover:bg-charcoal/50 transition-colors duration-300 flex flex-col items-center justify-center gap-3">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); shareToWhatsApp(item.src, waMsg(item.id), lang); }}
      ...
      <AnimatePresence>
        {lightbox !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <button
              onClick={(e) => { e.stopPropagation(); shareToWhatsApp(filtered[lightbox]?.src ?? '', waMsg(filtered[lightbox]?.id ?? ''), lang); }}
