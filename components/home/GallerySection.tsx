...
const RATIOS = ['aspect-[3/4]','aspect-square','aspect-[4/5]','aspect-[3/4]','aspect-square','aspect-[4/5]','aspect-[3/4]','aspect-square','aspect-[4/5]','aspect-[3/4]','aspect-square','aspect-[4/5]'];
        ...
        >
          {filtered.map((item, i) => (
            <motion.div key={item.id} variants={fadeUp}
              className={`group relative overflow-hidden rounded-sm bg-stone cursor-pointer ${RATIOS[i % RATIOS.length]}`}
              whileHover={{ y: -3 }} transition={{ duration: 0.3 }}
            >
              <button onClick={() => setLightbox(i)} className="absolute inset-0 w-full h-full">
                {isDataUrl(item.src) ? (
              ...
              <a href={waMsg(i)} target="_blank" rel="noopener noreferrer"
                ...
                className="absolute bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 flex items-center gap-1.5 bg-[#25D366] text-white text-xs font-semibold px-4 py-2 rounded-full shadow-lg whitespace-nowrap z-10"
                style={{ fontFamily: font }}>
        ...
        {lightbox !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <button
              ...
              className="absolute left-5 text-white/60 hover:text-white p-3 z-10 text-3xl">‹</button>
