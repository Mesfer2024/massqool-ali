    ...
    >
      ...
      <Link href={`/products/${product.slug}`} className="block relative aspect-[3/4] overflow-hidden bg-stone rounded-sm">
        <Image
          ...
          fill
          className={`object-cover group-hover:scale-105 transition-transform duration-700 ease-out${product.isSold ? ' opacity-75' : ''}`}
        ...
        {product.isSold && (
          <div
            className="absolute bottom-3 start-3 bg-charcoal/75 backdrop-blur-sm text-white text-[10px] px-2.5 py-1 rounded-full flex items-center gap-1"
            style={{ fontFamily: font }}
          >
      ...
      <div className="flex flex-col gap-3 pt-4 px-1" style={{ fontFamily: font }}>
        ...
        {/* Actions */}
        {product.isSold ? (
          <div className="flex items-center justify-center py-2.5 text-ink/40 text-xs font-semibold" style={{ fontFamily: font }}>
            {t('sold.badge')}
          </div>
        ) : (
